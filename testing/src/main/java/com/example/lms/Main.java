...
public class Main {
	...
	public static void main(String[] args) {
		...
		final Integer APP_PORT = Integer.parseInt(dotenv.get("APP_PORT", "7070"));
		...
		Javalin app = Javalin.create(config -> {
			...
			config.router.apiBuilder(() -> {
				...
				// Swagger UI
				get("/swagger", ctx -> {
					try (var inputStream = Main.class.getClassLoader().getResourceAsStream("public/swagger.html")) {
						if (inputStream == null) {
				...
				get("/swagger.json", ctx -> {
					try (var inputStream = Main.class.getClassLoader().getResourceAsStream("docs/swagger.json")) {
						if (inputStream == null) {
				...
				get("/docs/swagger.json", ctx -> {
					...
					try (var inputStream = Main.class.getClassLoader()
